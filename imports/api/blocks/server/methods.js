                    ...
                    if (response.statusCode == 200){
                        ...
                        blockData.hash = block.block_id.hash;
			                  if(block.block.data.txs != null){
			                      blockData.transNum = block.block.data.txs.length;
			                  } else {
			                      blockData.transNum = 0;
			                  }
                        ...
                        let signatures = block.block.last_commit.signatures;
                        if (signatures != null){
                            // console.log(precommits.length);
                            for (let i=0; i<signatures.length; i++){
                                if (signatures[i] != null){
                                    blockData.validators.push(signatures[i].validator_address);
                                }
                            }

                            analyticsData.precommits = signatures.length;
                        ...
                        if (height > 1){
                            ...
                            for (i in validators.result.validators){
                                ...
                                for (j in signatures){
                                    if (signatures[j] != null){
                                        if (address == signatures[j].validator_address){
                                            record.exists = true;
                                            signatures.splice(j,1);
                                            break;
                                        }
                                    }
                                }
                                        ...
                                        if (validatorSet[val].consensus_pubkey == valExist.consensus_pubkey){
                                            ...
                                            if (height % 30 == 1){
                                                try{
                                                    ...
                                                    if (response.statusCode == 200){
                                                        let sDel = JSON.parse(response.content);
                                                        selfDelegation = sDel.result;
