...
export default class VotingPower extends Component{
    ...
    componentDidUpdate(prevProps){
        if (prevProps.stats != this.props.stats){
                    ...
                    tooltips: {
                        callbacks: {
                            label: function(tooltipItem, data) {
                                return numbro(data.datasets[0].data[tooltipItem.index]).format(window.config.numbers.decimalFormat)+" ("+(numbro(data.datasets[0].data[tooltipItem.index]/totalVotingPower).format(window.config.numbers.percentWithDecimals)+", "+numbro(accumulatePower[tooltipItem.index]/totalVotingPower).format(window.config.numbers.percentWithDecimals))+")";
                            }
                        }
                    },
                        ...
                        xAxes: [{
                            ticks: {
                                ...
                                userCallback: function(value, index, values) {
                                    ...
                                    return numbro(value).format(window.config.numbers.decimalFormat);
                                }
                            }
                        }]
                    }
            ...
            $("#voting-power-chart").height(30+30*data.length);
        }
    }
