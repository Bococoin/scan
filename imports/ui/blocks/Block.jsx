        ...
        if (this.props.loading){
            ...
            if (this.props.blockExist){
                ...
                return <Container id="block" className="paddingleft">
                    <Helmet>
                        <title>Block {numbro(block.height).format(window.config.numbers.decimalFormat)} | BocoScan</title>
                        <meta name="description" content={"Block details of height "+numbro(block.height).format(window.config.numbers.decimalFormat)} />
                    </Helmet>
                    <h4><T>blocks.block</T> {numbro(block.height).format(window.config.numbers.decimalFormat)}</h4>
                    <Card>
                        ...
                        <CardBody>
                            <Row>
                                ...
                                <Col md={8} className="value">{numbro(block.transNum).format(window.config.numbers.decimalFormat)}</Col>
                                <Col md={4} className="label"><T>common.time</T></Col>
                                <Col md={8} className="value">{moment.utc(block.time).format(window.config.dateTimeFormat.defaultZ)} ({moment(block.time).fromNow()})</Col>
                            </Row>
                        </CardBody>
                    </Card>
                    <TransactionTabs 
