...
export default ValidatorDetailsContainer = withTracker((props) => {
    ...
    let loading = true;
    const isMovementOnly = window.location.pathname.indexOf('/movement') !== -1;
    ...
    if (Meteor.isServer || !loading){
        const query = isMovementOnly ? { 'tx.value.msg.type': 'cosmos-sdk/MsgSend' } : {};
        transactions = Transactions.find(query, {sort:{height:-1}}).fetch();
