        ...
        if (this.props.proposal != prevProps.proposal){
            ...
            if (this.props.proposal.voting_start_time != '0001-01-01T00:00:00Z'){
                if (now.diff(moment(this.props.proposal.voting_start_time)) > 0){
                    ...
                    //         tallyDate: moment.utc(this.props.proposal.updatedAt).format(window.config.dateTimeFormat.defaultZ),
                    //         voteStarted: true,
                    //         voteEnded: false,
                ...
                {votes.map((vote, i) =>
                    <Card body key={i}><Row className='voter-info'>
                        ...
                        <Col className="voting-power data" md={4}>
                            ...
                            {(vote.votingPower!==undefined)?numbro(Math.sqrt(vote.votingPower)).format('0,0.00'):""}
                        </Col>
                        <Col className="voting-power-percent data" md={3}>
                            <i className="material-icons d-md-none">equalizer</i>
                            {(vote.votingPower!==undefined) && (this.state.totalVotes!=0) ?numbro((Math.sqrt(vote.votingPower))/this.state.totalVotes).format('0,0.00%'):"0.00%"}
                        </Col>
                    </Row></Card>
                )}
            </Card>
    ...
    render(){
        if (this.props.loading){
            ...
            if (this.props.proposalExist && this.state.proposal != ''){
                ...
                return <div>
                    <Helmet>
                        <title>{this.props.proposal.proposal_content.value.title} | BocoScan</title>
                    ...
                    <div className="proposal bg-light">
                        ...
                        <Row className="mb-2 border-top">
                            ...
                            <Col md={9} className="value">{this.props.proposal.ranking>0?this.props.proposal.ranking:'Not Eligible'}</Col>
                        </Row>
                        <Row className="mb-2 border-top">
                                        ...
                                        {this.props.proposal.deposits?this.props.proposal.deposits.map((deposit,i)=>{
                                            return <li key={i}>
                                                ...
                                                {deposit.amount.map((amount, j) => {
                                                    return <div key={j}>{numbro(amount.amount/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormat)} {Meteor.settings.public.stakingDenom}</div>
                                                })}
                                            </li>
                                        }):''}
                                    </ol>
                        ...
                        <Row className="mb-2 border-top tally-result">
                            ...
                            <Col md={9} className="value">
                                <Row>
                                    ...
                                    <Col xs={5} sm={6} md={7} className="tally-result-value">{this.state.tally?numbro(this.state.tally.yes).format(window.config.numbers.decimalFormat):''}</Col>
                                ...
                                <Row>
                                    ...
                                    <Col xs={5} sm={6} md={7} className="tally-result-value">{this.state.tally?numbro(this.state.tally.abstain).format(window.config.numbers.decimalFormat):''}</Col>
                                ...
                                <Row>
                                    ...
                                    <Col xs={5} sm={6} md={7} className="tally-result-value">{this.state.tally?numbro(this.state.tally.no).format(window.config.numbers.decimalFormat):''}</Col>
                                ...
                                {/* <Row>
                                    ...
                                    <Col xs={5} sm={6} md={7} className="tally-result-value">{this.state.tally?numbro(this.state.tally.no_with_veto).format(window.config.numbers.decimalFormat):''}</Col>
                                ...
                                {this.state.voteStarted?<Row>
                                    <Col xs={12}><Card>
                                        ...
                                        <CardBody>
                                            <TabContent activeTab={this.state.breakDownSelection=='Bar'?'bar':'pie'}>
                                                <TabPane tabId="bar">
                                                    <Progress multi>
                                                        <Progress bar animated color="success" value={this.state.yesPercent}><T>proposals.yes</T> {numbro(this.state.yesPercent).format(window.config.numbers.withDecimal)}%</Progress>
                                                        <Progress bar animated color="warning" value={this.state.abstainPercent}><T>proposals.abstain</T> {numbro(this.state.abstainPercent).format(window.config.numbers.withDecimal)}%</Progress>
                                                        <Progress bar animated color="danger" value={this.state.noPercent}><T>proposals.no</T> {numbro(this.state.noPercent).format(window.config.numbers.withDecimal)}%</Progress>
                                                        {/* <Progress bar animated color="info" value={this.state.noWithVetoPercent}><T>proposals.noWithVeto</T> {numbro(this.state.noWithVetoPercent).format(window.config.numbers.withDecimal)}%</Progress> */}
                                                    </Progress>
                                                </TabPane>
                                                <TabPane tabId="pie">
                                                    {this.renderPieChart()}
                                    ...
                                    {/* <Col xs={12}>
                                        <Card body className="tally-info">
                                            <em>
                                                <T _purify={false} percent={numbro(this.state.totalVotes/totalVotingPower).format(window.config.numbers.percentWithDecimals)}>proposals.percentageVoted</T><br/>
                                                {this.state.proposalValid?<T _props={{className:'text-success'}} tentative={(!this.state.voteEnded)?'(tentatively) ':''}_purify={false}>proposals.validMessage</T>:(this.state.voteEnded)?<T _props={{className:'text-danger'}} quorum={numbro(this.props.chain.gov.tallyParams.quorum).format(window.config.numbers.percentWithDecimals)} _purify={false}>proposals.invalidMessage</T>:<T moreVotes={numbro((totalVotingPower*this.props.chain.gov.tallyParams.quorum-this.state.totalVotes)/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormat)} _purify={false}>proposals.moreVoteMessage</T>}
                                            </em>
                                        </Card>
                                    </Col> */}
                        ...
                        <Row className="mb-2 border-top">
                            ...
                            <Col md={9} className="value">{moment.utc(this.state.proposal.submit_time).format(window.config.dateTimeFormat.defaultZ)}</Col>
                        </Row>
                        {/* <Row className="mb-2 border-top">
                            <Col md={3} className="label"><T>proposals.depositEndTime</T></Col>
                            <Col md={9} className="value">{moment.utc(this.state.proposal.deposit_end_time).format(window.config.dateTimeFormat.defaultZ)}</Col>
                        </Row> */}
                        <Row className="mb-2 border-top">
                            <Col md={3} className="label"><T>proposals.votingStartTime</T></Col>
                            <Col md={9} className="value">{(this.state.proposal.voting_start_time != '0001-01-01T00:00:00Z')?moment.utc(this.state.proposal.voting_start_time).format(window.config.dateTimeFormat.defaultZ):'-'}</Col>
                        </Row>
                        {/* <Row className="mb-2 border-top">
                            <Col md={3} className="label"><T>proposals.votingEndTime</T></Col>
                            <Col md={9} className="value">{(this.state.proposal.voting_start_time != '0001-01-01T00:00:00Z')?moment.utc(this.state.proposal.voting_end_time).format(window.config.dateTimeFormat.defaultZ):'-'}</Col>
                        </Row> */}
                    </div>
