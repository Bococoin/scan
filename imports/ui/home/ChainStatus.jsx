...
export default class ChainStatus extends React.Component {
    ...
    componentDidUpdate(prevProps){
        if (prevProps != this.props){
            ...
            this.setState({
                blockHeight: numbro(this.props.status.latestBlockHeight).format({thousandSeparated: window.config.thousandSeparated}),
                blockTime: moment.utc(this.props.status.latestBlockTime).format(window.config.dateTimeFormat.defaultZ),
                delegatedTokens: numbro(this.props.status.totalVotingPower).format(window.config.numbers.votingPowerFormat),
                ...
                totalSupply: this.props.states.totalLiquids[0].amount
            })
            ...
            case "":
                this.setState({
                    votingPower: numbro(this.props.status.activeVotingPower).format(window.config.numbers.votingPowerFormat),
                });
                break;
            case "h":
                this.setState({
                    votingPower: numbro(this.props.status.lastHourVotingPower).format(window.config.numbers.votingPowerFormat),
                });
                break;
            case "d":
                this.setState({
                    votingPower: numbro(this.props.status.lastDayVotingPower).format(window.config.numbers.votingPowerFormat),
                });
                break;
                    ...
                    <Row className="status text-center">
                        <Col md={4}>
                            <Card body className="shade">
                                ...
                                <CardText>
                                    <span className="display-4 value">{this.state.blockHeight}</span>
                        ...
                        {/* <Col md={3}>
                            <Card body className="shade">
                                ...
                                <CardText><span className="display-4 value">{this.state.votingPower}</span><T percent={numbro(this.state.bondedTokens/(this.state.totalSupply+this.state.communityPool)).format(window.config.numbers.percentWithDecimals)} totalStakes={numbro((this.state.totalSupply+this.state.communityPool)/Meteor.settings.public.stakingFraction).format(window.config.numbers.totalStakes)} denom={Meteor.settings.public.stakingDenom}>chainStatus.fromTotalStakes</T></CardText>
                            </Card>
                        </Col> */}
                        <Col md={4}>
                            <Card body className="shade">
                                <CardTitle><T>chainStatus.activeValidators</T></CardTitle>
                                <CardText><span className="display-4 value">{this.state.numValidators}</span><T totalValidators={this.state.totalNumValidators}>chainStatus.outOfValidators</T></CardText>
                            </Card>
                        </Col>
                        <Col md={4}>
                            <Card body className="shade">
                                ...
                                <CardText>
                                    <span className="display-4 value">{this.state.averageBlockTime}</span><T>chainStatus.seconds</T>
                                </CardText>   
                            </Card>
                        </Col>
                    </Row>
