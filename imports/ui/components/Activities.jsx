        ...
        return (
          <li key={i}>
            {data.coins.map((coin, j) => {
              ...
              return (
                <p key={j} className="text-success">
                  {numbro(
                    coin.amount / Meteor.settings.public.stakingFraction
                  ).format(window.config.numbers.decimalFormat)}{" "}
      ...
      {/* <ul>
                {props.msg.value.outputs.map((data,i) =>{
                    return <li key={i}><Account address={data.address}/> <T>activities.received</T> {data.coins.map((coin,j) =>{
                        return <em key={j} className="text-success">{numbro(coin.amount).format(window.config.numbers.decimalFormat)} {coin.denom}</em>
                    })}</li>
                })}
            </ul> */}
      {/* </p> */}
...
export default class Activites extends Component {
  ...
  render() {
    ...
    switch (msg.type) {
      ...
      case "cosmos-sdk/MsgCreateValidator":
        return (
          <span className="text-success">
            {numbro(
              msg.value.value.amount / Meteor.settings.public.stakingFraction
            ).format(window.config.numbers.decimalFormat)}{" "}
            {Meteor.settings.public.stakingDenom}
          </span>
        );
      ...
      case "cosmos-sdk/MsgDelegate":
        return (
          <span className="text-warning">
            {numbro(
              msg.value.amount.amount / Meteor.settings.public.stakingFraction
            ).format(window.config.numbers.decimalFormat)}{" "}
            {Meteor.settings.public.stakingDenom}
          </span>
        );
      // return <p><Account address={msg.value.delegator_address}/> {(this.props.invalid)?<T>activities.failedTo</T>:''}<MsgType type={msg.type} /> <em className="text-warning">{numbro(msg.value.amount.amount).format(window.config.numbers.decimalFormat)} {msg.value.amount.denom}</em> <T>activities.to</T> <Account address={msg.value.validator_address} /><T>common.fullStop</T></p>
      ...
      // return <p><Account address={msg.value.delegator_address} /> {(this.props.invalid)?<T>activities.failedTo</T>:''}<MsgType type={msg.type} /> <em className="text-warning">{numbro(msg.value.amount.amount).format(window.config.numbers.decimalFormat)} {msg.value.amount.denom}</em> <T>activities.from</T> <Account address={msg.value.validator_address} /><T>common.fullStop</T></p>
      ...
      // return <p><Account address={msg.value.delegator_address} /> {(this.props.invalid)?<T>activities.failedTo</T>:''}<MsgType type={msg.type} /> <em className="text-warning">{numbro(msg.value.amount.amount).format(window.config.numbers.decimalFormat)} {msg.value.amount.denom}</em> <T>activities.from</T> <Account address={msg.value.validator_src_address} /> <T>activities.to</T> <Account address={msg.value.validator_dst_address} /><T>common.fullStop</T></p>
      ...
      case "cosmos-sdk/MsgSubmitProposal":
        return (
          <span className="text-success">
            {numbro(
              msg.value.initial_deposit[0].amount /
                Meteor.settings.public.stakingFraction
            ).format(window.config.numbers.decimalFormat)}{" "}
            {Meteor.settings.public.stakingDenom}
          </span>
            ...
            {msg.value.amount.map((amount, i) => {
              ...
              if (i > 0) {
                return (
                  " ," +
                  numbro(
                    amount.amount / Meteor.settings.public.stakingFraction
                  ).format(window.config.numbers.decimalFormat) +
                  " " +
                  Meteor.settings.public.stakingDenom
                );
