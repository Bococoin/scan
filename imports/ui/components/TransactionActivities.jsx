            ...
            <ul>
                {props.msg.value.inputs.map((data,i) =>{
                    return <li key={i}><Account address={data.address}/> <T>activities.sent</T> {data.coins.map((coin, j) =>{
                        return <em key={j} className="text-success">{numbro(coin.amount/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormat)} {Meteor.settings.public.stakingDenom}</em>
                    })}
                    </li>
                })}
            </ul>
    ...
    render(){
        ...
        case "cosmos-sdk/MsgSend":
            ...
            for (let a in msg.value.amount){
                if (a > 0){
                    amount += ', '+numbro(msg.value.amount[a].amount/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormat)+" "+Meteor.settings.public.stakingDenom;
                }
                else{
                    amount += numbro(msg.value.amount[a].amount/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormat)+" "+Meteor.settings.public.stakingDenom;
                }
        ...
        case "cosmos-sdk/MsgSubmitProposal":
            return <p><Account address={msg.value.proposer} /> {(this.props.invalid)?<T>activities.failedTo</T>:''}<MsgType type={msg.type} /> <T>activities.withTitle</T>{(this.props.tags && this.props.tags.length > 1) ? <Link to={"/proposals/"+this.props.tags[2].value}>{msg.value.title}</Link> : ''} <T>common.fullStop</T></p>
        case "cosmos-sdk/MsgDeposit":
            return <p><Account address={msg.value.depositor} /> {(this.props.invalid)?<T>activities.failedTo</T>:''}<MsgType type={msg.type} /> <em className="text-info">{msg.value.amount.map((amount,i) =>{
                if (i>0){
                    return " ,"+numbro(amount.amount/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormat)+" "+Meteor.settings.public.stakingDenom;
                }
                else{
                    return numbro(amount.amount/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormat)+" "+Meteor.settings.public.stakingDenom;
                }
