...
export default class PowerHistory extends React.Component {
    constructor(props) {
        ...
        this.state = {
            tx : "",
            diff: <span className={"text-"+((props.votingPower - props.prevVotingPower>0)?"success":"danger")+" vp-diff"}>({numbro(props.votingPower - props.prevVotingPower).format(window.config.numbers.withPlus)})</span>
        }
            ...
            <Card className={this.props.type}>
                <CardBody>
                    <Row>
                        ...
                        <Col xs={{size:10, offset:2}} sm={{offset:0, size:4}} className="text-secondary"><i className="fas fa-cube"></i> {numbro(this.props.height).format('0,0')}<br/><i className="far fa-clock"></i> {momemt.utc(this.props.time).format(window.config.dateTimeFormat.defaultZ)}</Col>
                    </Row>
                </CardBody>
                {this.state.tx}
            </Card>
