...
export default class AccountDetails extends Component{
    ...
    getBalance(){
        ...
        Meteor.call('accounts.getAddressBalance', this.props.match.params.address, (error, result) => {
            if (error){
                console.warn(error);
                this.setState({
                    loading:false
    ...
    render(){
        ...
        if (this.state.loading){
            ...
            <div id="account" style={{
                ...
                <Helmet>
                    <title>Account Details of {this.state.address} | BocoScan</title>
                    <meta name="description" content={"Account Details of "+this.state.address+" on BocoScan"} />
                </Helmet>
                <Row>
                            ...
                            <Row>
                                <Col md={6} lg={8}>
                                    <Row>
                                        ...
                                        <Col xs={8} className="value text-right">{numbro(this.state.available/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormatWithSeparator)} {Meteor.settings.public.stakingDenom}</Col>
                                    </Row>
                                    <Row>
                                        <Col xs={4} className="label text-nowrap"><div className="delegated infinity" /><T>accounts.delegated</T></Col>
                                        <Col xs={8} className="value text-right">{numbro(this.state.delegated/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormatWithSeparator)} {Meteor.settings.public.stakingDenom}</Col>
                                    </Row>
                                    <Row>
                                        <Col xs={4} className="label text-nowrap"><div className="unbonding infinity" /><T>accounts.unbonding</T></Col>
                                        <Col xs={8} className="value text-right">{numbro(this.state.unbonding/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormatWithSeparator)} {Meteor.settings.public.stakingDenom}</Col>
                                    </Row>
                                    <Row>
                                        <Col xs={4} className="label text-nowrap"><div className="rewards infinity" /><T>accounts.rewards</T></Col>
                                        <Col xs={8} className="value text-right">{numbro(this.state.rewards/Meteor.settings.public.stakingFraction).format(window.config.numbers.decimalFormatWithSeparator)} {Meteor.settings.public.stakingDenom}</Col>
                                    </Row>
                                </Col>
                                <Col md={6} lg={4} className="total d-flex flex-column justify-content-end">
                                    <Row>
                                        ...
                                        <Col xs={8} className="value text-right">{numbro(this.state.total/Meteor.settings.public.stakingFraction).format(window.config.numbers.stakingDenom)} {Meteor.settings.public.stakingDenom}</Col>
                                        {/* <Col xs={12} className="dollar-value text-right text-secondary">~{numbro(this.state.total/Meteor.settings.public.stakingFraction*this.state.price).format(window.config.numbers.dollaredLong)} ({numbro(this.state.price).format(window.config.numbers.dollarFormat)}/{Meteor.settings.public.stakingDenom})</Col> */}
                                    </Row>
                                </Col>
                            </Row>
                        </CardBody>
